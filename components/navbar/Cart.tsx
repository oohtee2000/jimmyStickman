...
const Cart = () => {
  ...
  return (
    <div className="p-6 min-h-screen flex flex-col items-center bg-gray-100">
      <h2 className="text-3xl font-bold mb-6 text-gray-800 self-start">Shopping Cart</h2>
      <div className="bg-white p-6 w-full max-w-full flex-1 overflow-y-auto max-h-[60vh]">
        {items.length > 0 ? (
          items.map((item) => (
            <div
              key={item.id}
            ...
            >
              <img
                src={item.image}
                alt={item.name}
                className="w-28 h-[154px] object-cover rounded-lg"
              />
              <div className="ml-6 flex-1 flex flex-col justify-between h-full">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center bg-gray-200 rounded-lg px-2 py-1">
                    ...
                    <button
                      className="p-2 text-gray-700 hover:text-gray-900 mr-4"
      ...
      <div className="w-full text-center bg-white sticky bottom-0 ">
        <div className="flex justify-between font-semibold text-gray-800 p-4">
          <p>Subtotal <span className="text-gray-500 text-sm">({items.length} Items)</span></p>
          <p>₦{items.reduce((total, item) => total + item.price * item.quantity, 0).toLocaleString()}</p>
        </div>
